...
function GetPayrollList($action, $dtFrom, $dtTo,$location,$empCode){
    ...
    WHERE tot_days_work is not null and a.emp_status = :status and period_from = :period_from AND period_to = :period_to and b.location = :location ORDER BY a.lastname ASC';
...
function GetPayrollEmpList($action, $dtFrom, $dtTo,$location,$empCode,$emp_code){
    ...
    from employee_profile a left join
    att_summary b on a.badgeno = b.badge_no
    WHERE tot_days_work is not null and a.emp_status = :status and period_from = :period_from AND period_to = :period_to and b.location = :location and a.badgeno = :emp_code ORDER BY a.lastname ASC';
    $param = array(":period_from" => $dtFrom, ":period_to" => $dtTo,":location" => $location, ":status" => 'Active',":emp_code" => $emp_code);
    $stmt =$connL->prepare($query);
    $stmt->execute($param);
    $r = $stmt->fetch();
    ...
    <input type='text' id='myInput' onkeyup='myFunction()' placeholder='Search for names..' title='Type in a name'>
    <table id='payrollEmpList' class='table table-striped table-sm table3'> 
    <thead>
    <tr>
    <th >Last Name</th>
    <th>First Name</th>
