...
function GenOt($eMplogName,$pyrollco_from,$pyrollco_to){
    ...
    $cmd2 = $connL->prepare('EXEC test_hris.dbo.GenerateActualOtRendered :pay_from,:pay_to');
    $cmd2->bindValue(':pay_from',$pyrollco_from);
    $cmd2->bindValue(':pay_to',$pyrollco_to);
    $cmd2->execute();

    $cmd = $connL->prepare('EXEC test_hris.dbo.GenerateOTToAttendance :pay_from,:pay_to');
